import { instantiateWasmModule } from '../../lib/node/wasm';
import { expect, test } from 'vitest';

test('should be able to detect cyclies in any directed graph', async () => {
...
test('should be able to topologically sort any directed acyclic graph', async () => {
  const wasmModule = await instantiateWasmModule();
  ...
  expect(topologicallySorted3).to.deep.equal([
    0, 5, 9, 18, 10, 14, 19, 20, 21, 15, 16, 17, 11, 1, 6, 2, 3, 4, 13, 12, 8, 7
  ]);
});
